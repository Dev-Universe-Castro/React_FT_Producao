...
const Stock = () => {
    const { data, updateStockLevel, addStockMovement } = useData();
    ...
    const handleExitSubmit = (e) => {
        e.preventDefault();
        if (!selectedProduct || !exitQuantity || exitQuantity <= 0) {
        ...
        const newStock = currentStock - Number(exitQuantity);
        updateStockLevel(selectedProduct.id, newStock);
        addStockMovement({
            id: `sm-${Date.now()}`,
            ...
            reason: exitReason || 'Saída Manual',
        });
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <Helmet>
                <title>Posição de Estoque | FT Produção</title>
            </Helmet>
